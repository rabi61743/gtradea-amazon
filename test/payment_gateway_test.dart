...
PlacedOrder order(Map<String, dynamic> gateway) =>
...
void main() {
  ...
  group('recognising the return', () {
    test('accepts the configured return host and rejects a lookalike', () {
      ...
      expect(isReturnUrl(Uri.parse('https://gtradea.com/payment/success')),
          isTrue);
      expect(isReturnUrl(Uri.parse('https://gtradea.com/payment-callback?x=1')),
          isTrue);
  ...
  group('khalti', () {
    ...
    test('opens the hosted page it was given', () {
      final launch = gateway.launch(order({
        'pidx': 'PIDX1',
        'paymentUrl': 'https://khalti.test/pay/PIDX1',
      }));
    ...
    test('looks up the pidx the gateway returned, not the one it was given',
        () async {
      // Khalti appends the pidx it actually processed. Asking about the
      // original would check the wrong payment.
      api.on('POST', '/payments/khalti/lookup',
    ...
    test('a payment under review is terminal, and never offers a retry',
        () async {
  ...
  group('esewa', () {
    ...
    test('posts the signed bag, under either name the server uses', () {
      final withFormFields = gateway.launch(order(const {
        'actionUrl': 'https://esewa.test/pay',
        'formFields': {'amt': '100'},
      }));
      expect(withFormFields.formFields, const {'amt': '100'});

      final withFields = gateway.launch(order(const {
        'actionUrl': 'https://esewa.test/pay',
        'fields': {'amt': '200'},
      }));
    ...
    test('sends the base64 envelope back verbatim', () async {
      ...
      final verdict =
          await gateway.confirm(order(const {}), const {'data': 'eyJhIjoxfQ=='});
    ...
    test('needs only success, because its verify carries no status', () async {
      ...
      api.on('POST', '/payments/esewa/verify',
          body: const {'success': true, 'status': null});

      final verdict =
          await gateway.confirm(order(const {}), const {'data': 'x'});
      expect(verdict.paid, isTrue);
    });

    test('a return with no envelope is reported, not sent as empty', () async {
      await expectLater(
        gateway.confirm(order(const {}), const {}),
        throwsA(isA<ApiError>()),
      );
  ...
  group('connectips', () {
    ...
    test('posts the bag under its own key', () {
      final launch = gateway.launch(order(const {
    ...
    test('prefers the txnId it was given over the one returned', () async {
      ...
      api.on('POST', '/payments/connectips/check-status',
          body: const {'success': true, 'status': 'success'});

      await gateway.confirm(
        order(const {'txnId': 'FROM-INITIATE'}),
        const {'TXNID': 'FROM-RETURN'},
      );
  ...
  group('nps', () {
    ...
    test('lifts the merchant transaction id out of the signed bag', () async {
      api.on('POST', '/payments/nps/check-status',
          body: const {'success': true, 'status': 'success'});
    ...
    test('a customer who never started is a back-out, not a failure',
        () async {
  ...
  group('fonepay', () {
    ...
    test('sends the whole return map back untouched', () async {
      ...
      // invalidates it.
      api.on('POST', '/payments/fonepay/verify',
          body: const {'success': true, 'status': 'success'});
  ...
  group('the order poll', () {
    test('reads paid, failed and still-waiting from the order itself',
        () async {
      Future<GatewayVerdict> withOrder(String status, String payment) {
        api.on('GET', '/orders/order-1',
